...
::
:: Script para executar o comando lasboundary64 do lastools em multiplos ficheiros numa lista de pastas/caminhos
:: https://downloads.rapidlasso.de/html/lasboundary_README.html
::
...
REM opção -keep_class 2 - Apenas manter classe 2 = Ground
REM opção de -v verbose, relatar a execução de cada ficheiro
set opcoes=-epsg 3763 -keep_class 2 -v
set ncores=16
...
for /f "skip=1 usebackq tokens=1,2 delims=," %%a in (%lotelist%) do (
  ...
  IF EXIST %boundariesdir%\%%a.gpkg (echo done) ELSE (
    ...
    lasboundary64 %opcoes% -labels -labels_file_name_full -only_2d -cores %ncores% -i %%b\*.laz -odir %boundariesdir%\boundaries_%%a
